...
module.exports = async() => {
    ...
    try {
        ...
        const existingCommands = await rest.get(Routes.applicationGuildCommands(process.env.CLIENT_ID, process.env.FD_DC_ID));
        
        for (const command of existingCommands) {
            await rest.delete(Routes.applicationGuildCommand(process.env.CLIENT_ID, process.env.FD_DC_ID, command.id));
        }
        
        console.log('Registrujem príkazy...🟡');
        await rest.put(Routes.applicationGuildCommands(process.env.CLIENT_ID, process.env.FD_DC_ID), { body: commands });
